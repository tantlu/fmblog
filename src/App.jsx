...
const DemoModeAlert = () => (
  <div className="fixed bottom-4 right-4 z-[200] bg-white/90 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl shadow-2xl backdrop-blur-md flex items-center gap-3 animate-in slide-in-from-bottom-5">
...
const PaymentModal = ({ product, onClose, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm">
      <div className="bg-white border border-amber-100 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl">
        <div className="p-6 bg-gradient-to-br from-amber-50 to-white">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-amber-600 text-sm mt-1 font-medium">{product.name}</p>
            </div>
          ...
          <button onClick={() => { alert('Cảm ơn bạn đã ủng hộ Blog!'); onClose(); }} className="w-full mt-6 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white font-bold py-3 rounded-xl transition shadow-lg shadow-amber-200 flex items-center justify-center gap-2">
...
// 2. Rich Text Editor (Pro Version)
const RichTextEditor = ({ value, onChange }) => {
  ...
  const ToolBtn = ({ onClick, icon: Icon, title }) => (
    <button type="button" onClick={onClick} className="p-2 hover:bg-slate-200 rounded text-slate-600 transition hover:text-amber-600" title={title}>
      <Icon size={18} />
    </button>
  );

  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm flex flex-col h-[500px]">
      ...
      <div
        ref={editorRef}
        className="flex-1 p-6 overflow-y-auto text-slate-800 focus:outline-none prose prose-slate max-w-none prose-headings:font-serif prose-headings:text-slate-900 prose-a:text-amber-600 prose-img:rounded-xl"
        contentEditable
        onInput={handleInput}
...
const Navbar = ({ user, setView, currentView, setCategoryFilter, currentFilter, handleLogout }) => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40 transition-all duration-300 shadow-sm">
        ...
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center cursor-pointer group" onClick={() => { setView('home'); setCategoryFilter(null); }}>
            ...
            <div className="flex flex-col">
              <span className="text-xl font-serif font-bold text-slate-900 tracking-wide leading-none">FM<span className="text-amber-500">BLOG</span></span>
              <span className="text-[10px] text-slate-400 uppercase tracking-[0.2em] font-medium">Personal Journal</span>
            </div>
          </div>
            ...
            {user ? (
              <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                <div className="text-right hidden xl:block">
                  ...
                  <div className="text-sm font-serif font-bold text-slate-900">{getUserDisplayName(user)}</div>
                </div>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-slate-100 absolute w-full z-50 shadow-xl animate-in slide-in-from-top-5">
          <div className="px-4 pt-2 pb-6 space-y-1">
            {navItems.map(item => (
              <button key={item.id} onClick={() => { item.action(); setIsOpen(false); }} className={`block w-full text-left px-3 py-4 rounded-xl text-base font-medium ${isActive(item) ? 'bg-amber-50 text-amber-700' : 'text-slate-600 hover:bg-slate-50'}`}>
                <span className="flex items-center gap-3">{item.icon} {item.label}</span>
              </button>
            ))}
            {user?.email === ADMIN_EMAIL && (
              <button onClick={() => { setView('admin'); setIsOpen(false) }} className="block w-full text-left px-3 py-4 rounded-xl text-base font-medium text-amber-700 hover:bg-slate-50">Admin Dashboard</button>
            )}
            <div className="border-t border-slate-100 my-2"></div>
            {user ? (
              <button onClick={() => { handleLogout(); setIsOpen(false); }} className="block w-full text-left px-3 py-4 rounded-xl text-base font-medium text-red-500 hover:bg-red-50">Đăng xuất</button>
            ) : (
              <button onClick={() => { setView('login'); setIsOpen(false) }} className="block w-full text-left px-3 py-4 rounded-xl text-base font-medium text-slate-900 font-bold bg-slate-50 mt-2">Đăng nhập thành viên</button>
            )}
          </div>
        </div>
      )}
...
// 4. Article Card (Blog Style - White & Minimal)
const ArticleCard = ({ article, onClick }) => (
  <div onClick={() => onClick(article)} className="group bg-white rounded-xl overflow-hidden border border-slate-100 hover:border-amber-200 cursor-pointer transition-all duration-300 hover:shadow-xl hover:shadow-amber-50/50 flex flex-col h-full">
    <div className="h-60 overflow-hidden relative">
      ...
      <div className="absolute top-4 left-4 bg-white/90 backdrop-blur text-slate-900 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm uppercase tracking-wider border border-slate-100">
        {article.category || 'Journal'}
      </div>
    </div>
    <div className="p-6 flex flex-col flex-1">
      <div className="mb-3 flex items-center text-xs text-amber-600 font-medium uppercase tracking-widest">
        {new Date(article.createdAt?.seconds * 1000).toLocaleDateString('vi-VN')}
      </div>
      <h3 className="text-xl font-serif font-bold text-slate-900 mb-3 leading-snug group-hover:text-amber-600 transition-colors">{article.title}</h3>
      <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
        ...
        <span className="text-slate-400 hover:text-amber-600 text-xs font-bold flex items-center gap-1 transition-colors">Đọc tiếp &rarr;</span>
      </div>
    </div>
  </div>
);

// 5. Store Component (Luxury Gold Style)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      if (prods.length === 0 && user && user.uid) {
        // Seeding logic omitted for brevity, same as before
      }
    }, (err) => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-16 px-4">
      ...
      <div className="text-center mb-20 relative">
        ...
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-slate-900 mb-6">Cửa Hàng Độc Quyền</h2>
        <p className="text-slate-500 max-w-2xl mx-auto text-lg font-light">Nâng cấp trải nghiệm quản lý bóng đá của bạn với các công cụ và dữ liệu được chọn lọc kỹ càng.</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.map(product => (
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden border border-slate-100 hover:border-amber-300 transition-all duration-500 group hover:-translate-y-2 shadow-sm hover:shadow-2xl hover:shadow-amber-100/50 flex flex-col">
              <div className="h-56 p-8 bg-slate-50 flex items-center justify-center relative group-hover:bg-white transition-colors">
                ...
                {product.image && product.image.startsWith('http') ? (
                  ...
                  <Crown className="text-amber-400 w-20 h-20 group-hover:text-amber-500 transition-colors z-10" />
                )}
                <div className={`absolute top-4 right-4 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider z-20 bg-white shadow-sm border border-slate-100 text-slate-500`}>
                  {product.type === 'game' ? 'KEY' : 'MOD'}
                </div>
              </div>
              <div className="p-6 flex-1 flex flex-col text-center">
                <h3 className="text-lg font-serif font-bold text-slate-900 mb-2">{product.name}</h3>
                <p className="text-slate-500 text-sm mb-6 flex-1 line-clamp-2 font-light">{product.description}</p>
                <div className="mt-auto">
                  <div className="text-2xl font-bold text-slate-900 mb-4">{parseInt(product.price).toLocaleString('vi-VN')} <span className="text-sm text-slate-400 font-normal align-top">đ</span></div>
                  <button onClick={() => setSelectedProduct(product)} className="w-full bg-slate-900 hover:bg-amber-600 text-white py-3 rounded-xl font-bold text-sm transition-colors shadow-lg shadow-slate-200 group-hover:shadow-amber-200 flex items-center justify-center gap-2">
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4">
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-3xl font-serif font-bold text-slate-900">Quản Trị Blog</h1>
        <div className="bg-amber-50 text-amber-700 px-4 py-2 rounded-full border border-amber-100 text-sm font-bold flex items-center gap-2">
          <Settings size={16} /> Admin Control: {user.email}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2 pb-4 border-b border-slate-100"><Edit3 className="text-amber-500" /> Viết Bài Mới (Pro Editor)</h3>
            <form onSubmit={handleCreateArticle} className="space-y-6">
              <div>
                ...
                <input required value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition font-serif text-lg placeholder:text-slate-300" placeholder="Tiêu đề bài viết..." />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-slate-500 text-xs font-bold uppercase mb-2 tracking-wider">Chuyên mục</label>
                  <select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 outline-none focus:border-amber-500">
                ...
                <div>
                  ...
                  <input value={image} onChange={e => setImage(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 outline-none focus:border-amber-500 placeholder:text-slate-300" placeholder="https://..." />
                </div>
              </div>
              <div>
              ...
              <button type="submit" className="w-full bg-slate-900 hover:bg-amber-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-slate-200 transition-all">Xuất Bản Bài Viết</button>
            </form>
          </div>
        </div>
        ...
        <div>
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm sticky top-24">
            ...
            <form onSubmit={handleAddProduct} className="space-y-5">
              ...
              <button type="submit" className="w-full bg-white border border-slate-200 hover:border-amber-500 text-slate-900 hover:text-amber-600 py-3 rounded-xl font-bold transition-all">Lưu Vào Kho</button>
            </form>
...
// 7. Article Detail (Blog Post Reading Mode)
const ArticleDetail = ({ article, onBack, user }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4 animate-in fade-in duration-700">
      <button onClick={onBack} className="mb-8 px-5 py-2 rounded-full border border-slate-200 text-slate-500 hover:text-slate-900 hover:border-slate-900 transition flex items-center gap-2 text-sm font-bold group">
        &larr; Quay lại <span className="group-hover:translate-x-1 transition-transform">Trang chủ</span>
      </button>

      <article className="mb-16">
        <div className="text-center mb-10">
          <span className="text-amber-600 font-bold tracking-widest text-xs uppercase mb-4 block">{article.category}</span>
          <h1 className="text-3xl md:text-5xl font-serif font-bold text-slate-900 leading-tight mb-6">{article.title}</h1>
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl shadow-slate-200 mb-12">
          <img src={article.image || "https://placehold.co/800x400"} alt={article.title} className="w-full h-auto object-cover" />
        </div>

        <div className="prose prose-lg prose-slate max-w-none prose-headings:font-serif prose-a:text-amber-600 prose-img:rounded-xl text-slate-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: article.content }} />
      </article>

      <div className="border-t border-slate-100 pt-12">
        <h3 className="text-2xl font-serif font-bold text-slate-900 mb-8">Thảo luận ({comments.length})</h3>

        {user ? (
          <form onSubmit={handlePostComment} className="mb-12">
            <textarea
              value={newComment} onChange={(e) => setNewComment(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 focus:bg-white focus:ring-1 focus:ring-amber-500 outline-none transition min-h-[120px] resize-none placeholder:text-slate-400"
              placeholder="Chia sẻ suy nghĩ của bạn..."
            />
            <div className="flex justify-end mt-3">
              <button type="submit" className="bg-slate-900 hover:bg-amber-600 text-white px-6 py-2 rounded-full font-bold text-sm transition">Gửi bình luận</button>
            </div>
          </form>
        ) : (
          <div className="bg-slate-50 p-8 rounded-2xl text-center mb-12 border border-slate-100">
        ...
        <div className="space-y-8">
          {comments.map(comment => (
            <div key={comment.id} className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-serif font-bold border border-slate-200">
                {comment.userName.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1">
                <div className="flex justify-between items-baseline mb-1">
                  <span className="font-bold text-slate-900">{comment.userName}</span>
                  <span className="text-xs text-slate-400">{comment.createdAt ? new Date(comment.createdAt.seconds * 1000).toLocaleDateString('vi-VN') : 'Vừa xong'}</span>
                </div>
...
const AuthModal = ({ setView, onLoginSuccess }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh] p-4">
      <div className="bg-white p-10 rounded-3xl shadow-2xl shadow-slate-200 border border-slate-100 w-full max-w-md relative">
        <button onClick={() => setView('home')} className="absolute top-4 right-4 text-slate-400 hover:text-slate-900"><X size={20} /></button>
        <div className="text-center mb-8">
          <div className="inline-block p-3 bg-amber-50 rounded-full mb-4"><User size={32} className="text-amber-600" /></div>
          <h2 className="text-2xl font-serif font-bold text-slate-900">{isRegister ? 'Đăng Ký Thành Viên' : 'Chào Mừng Trở Lại'}</h2>
          <p className="text-slate-500 text-sm mt-2">Tham gia cộng đồng FM Blog Việt Nam</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {isRegister && (
            <input required type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition placeholder:text-slate-400" placeholder="Tên hiển thị của bạn" />
          )}
          <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition placeholder:text-slate-400" placeholder="Email" />
          <input required type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition placeholder:text-slate-400" placeholder="Mật khẩu" />

          {error && <p className="text-red-500 text-sm text-center bg-red-50 p-2 rounded-lg">{error}</p>}

          <button type="submit" className="w-full bg-slate-900 hover:bg-amber-600 text-white py-4 rounded-xl font-bold text-lg transition-all shadow-lg shadow-slate-200">
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 text-center">
          <p className="text-xs text-slate-400">Tài khoản Admin Demo: {ADMIN_EMAIL}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-amber-200 selection:text-slate-900 overflow-x-hidden">
      ...
      <main className="pb-20">
        {view === 'home' && (
          <>
            {!categoryFilter && (
              <div className="relative bg-white border-b border-slate-100 mb-16 overflow-hidden">
                {/* Background Decor */}
                <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-amber-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 opacity-60 pointer-events-none"></div>

                <div className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center relative z-10">
                  <div className="md:w-1/2 mb-12 md:mb-0 animate-in slide-in-from-left-10 fade-in duration-700">
                    <div className="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-500 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-8 shadow-sm">
                      <Star size={12} className="text-amber-500" /> Nhật ký quản lý bóng đá
                    </div>
                    <h1 className="text-5xl md:text-7xl font-serif font-bold text-slate-900 tracking-tight mb-6 leading-[1.1]">
                      Chia sẻ đam mê <br />
                    ...
                    <div className="flex gap-4">
                      <button onClick={() => setView('store')} className="bg-slate-900 hover:bg-amber-600 text-white px-8 py-4 rounded-full font-bold transition-all shadow-xl shadow-slate-200 hover:shadow-amber-100 transform hover:-translate-y-1 flex items-center gap-2">
                        <ShoppingCart size={18} /> Ghé Cửa Hàng
                      </button>
                      <button className="bg-white hover:bg-slate-50 text-slate-900 px-8 py-4 rounded-full font-bold transition border border-slate-200 flex items-center gap-2">
                        <BookOpen size={18} /> Đọc Blog
                      </button>
                    </div>
                  </div>
                  <div className="md:w-1/2 flex justify-center relative animate-in zoom-in duration-1000">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
              <div className="flex items-center justify-between mb-12">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-1 bg-gradient-to-b from-amber-400 to-yellow-600 rounded-full"></div>
                  <h2 className="text-4xl font-serif font-bold text-slate-900">{categoryFilter || "Bài Viết Mới Nhất"}</h2>
                </div>
                {!categoryFilter && <button className="text-amber-600 text-sm font-bold hover:text-amber-700 flex items-center gap-1 border-b-2 border-amber-100 hover:border-amber-600 pb-1 transition-all">Xem toàn bộ <span className="text-lg">&rarr;</span></button>}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredArticles.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-24 bg-white rounded-3xl border border-slate-100 border-dashed shadow-sm">
                    <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
                      <BookOpen size={32} className="text-slate-300" />
                    </div>
                    <p className="text-slate-400 text-lg font-light">Chưa có bài viết nào trong mục này.</p>
        ...
        {view === 'admin' && (user?.email === ADMIN_EMAIL ? <AdminDashboard user={user} /> : <div className="flex items-center justify-center h-[60vh] text-slate-400">Bạn cần quyền Admin ({ADMIN_EMAIL}) để truy cập.</div>)}
      ...
      <footer className="bg-white border-t border-slate-100 py-12 text-center text-slate-500 text-sm">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-center items-center gap-3 mb-8">
            ...
            <span className="font-serif font-bold text-2xl text-slate-900 tracking-widest">FM BLOG</span>
          </div>
          <div className="flex justify-center gap-8 mb-8 font-medium text-slate-600">
          ...
          <p className="font-light text-slate-400">&copy; 2024 FM Blog Vietnam. Built with passion.</p>
        </div>
      </footer>
    </div>
  );
